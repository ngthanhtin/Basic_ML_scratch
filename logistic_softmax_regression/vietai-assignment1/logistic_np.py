...
class LogisticClassifier(object):
    ...
    def get_grad(self, x, y, y_hat):
        ...
        w_grad = np.dot((y_hat - y).T, x)
        w_grad /= y_hat.shape[0]
        return w_grad.T
    ...
    def update_weight(self, grad, learning_rate):
        ...
        self.w = self.w - np.dot(learning_rate, grad)
    ...
    def update_weight_momentum(self, grad, learning_rate, momentum, momentum_rate):
        ...
        momentum = np.dot(momentum_rate, momentum) + np.dot(learning_rate, grad)
...
def normalize_per_pixel(train_x, test_x):
    ...
    # train_x = ...
    # test_x = ...
    train_mean = np.zeros((1, train_x.shape[1], train_x.shape[2]))
    train_std = np.zeros((1, train_x.shape[1], train_x.shape[2]))
    
    #calculate std and mean for train_x
    for x in range(train_x.shape[1]):
        for y in range(train_x.shape[2]):
            for i in range(train_x.shape[0]):
                train_mean[0][x][y]+=train_x[i][x][y]
            train_mean[0][x][y] = train_mean[0][x][y] / train_x.shape[0]
            
            for i in range(train_x.shape[0]):
                train_std[0][x][y] += (train_x[i][x][y] - train_mean[0][x][y])**2
            train_std[0][x][y] = np.sqrt(train_std[0][x][y] / train_x.shape[0])
        
    #normalize train_x
    for x in range(train_x.shape[1]):
        for y in range(train_x.shape[2]):
            for i in range(train_x.shape[0]):
                train_x[i][x][y] = (train_x[i][x][y] - train_mean[0][x][y]) / train_std[0][x][y]
                
    #normalize test_x
    for x in range(test_x.shape[1]):
        for y in range(test_x.shape[2]):
            for i in range(test_x.shape[0]):
                test_x[i][x][y] = (test_x[i][x][y] - train_mean[0][x][y]) / train_std[0][x][y]
...
def normalize_all_pixel(train_x, test_x):
    ...
    #train_x
    mRC_train = (train_x.shape[0] * train_x.shape[1] * train_x.shape[2])
    train_mean = 0
    train_std = 0
    for i in range(train_x.shape[0]):
        for x in range(train_x.shape[1]):
            for y in range(train_x.shape[2]):
                train_mean+= train_x[i][x][y]
    train_mean = train_mean / mRC_train
    
    for i in range(train_x.shape[0]):
        for x in range(train_x.shape[1]):
            for y in range(train_x.shape[2]):
                train_std += (train_x[i][x][y] - train_mean)**2
    ...
    for i in range(train_x.shape[0]):
        for x in range(train_x.shape[1]):
            for y in range(train_x.shape[2]):
                train_x[i][x][y] = (train_x[i][x][y] - train_mean) / train_std
    ...
    for i in range(test_x.shape[0]):
        for x in range(test_x.shape[1]):
            for y in range(test_x.shape[2]):
                test_x[i][x][y] = (test_x[i][x][y] - train_mean) / train_std
    return train_x, test_x
...
def add_one(x):
    ...
    """
    # [TODO 1.4]
    v1 = np.ones((x.shape[0], 1))
    x = np.concatenate((x, v1), axis = 1)
    return x
