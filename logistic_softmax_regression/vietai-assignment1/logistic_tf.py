...
if __name__ == "__main__":
    ...
    x = tf.placeholder(dtype = tf.float64, shape = [None, train_x.shape[1]])
    
    y = tf.placeholder(dtype = tf.float64, shape = [None, train_y.shape[1]])
    ...
    x_entropy = tf.nn.sigmoid_cross_entropy_with_logits(logits=tf.matmul(x, w), labels=y)
